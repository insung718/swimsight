    ...
    <section className="stitch-panel min-w-0 p-4 lg:p-5">
      <div className="mb-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-white/42">Tiny nudges from your training data</p>
        </div>
      </div>

      <div className="space-y-2">
        {tips.map((tip) => (
          <article className="rounded-lg bg-white/[0.06] p-3" key={tip.id}>
            <h3 className="font-semibold text-white">{tip.title}</h3>
            <p className="mt-1 text-sm text-white/58">{tip.body}</p>
          </article>
        ))}
        {!tips.length && (
          <div className="rounded-lg border border-dashed border-white/10 p-5 text-sm text-white/38">
