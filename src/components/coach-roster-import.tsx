  ...
  return (
    <section className="dashboard-glass p-5">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        <label className="text-xs font-semibold uppercase text-white/48">{t("Club")}<select className="mt-2 h-10 w-full rounded-md border border-white/12 bg-[#0b1725] px-3 text-sm text-white" value={teamId} onChange={(event) => { setTeamId(event.target.value); setPreview(null); }}>{clubs.map((club) => <option data-no-translate key={club.id} value={club.id}>{club.name}</option>)}</select></label>
        <label className="text-xs font-semibold uppercase text-white/48">{t("Pilot cohort")}<select className="mt-2 h-10 w-full rounded-md border border-white/12 bg-[#0b1725] px-3 text-sm text-white" value={cohortId} onChange={(event) => { setCohortId(event.target.value); setPreview(null); }}><option value="">{t("Select cohort")}</option>{cohorts.map((cohort) => <option data-no-translate key={cohort.id} value={cohort.id}>{cohort.name}</option>)}</select></label>
