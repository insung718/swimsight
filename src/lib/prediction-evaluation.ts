...
export function projectPredictionToDate(prediction: Prediction, targetDate: string): ForecastProjection | null {
  ...
  const points = [
    { day: 0, time: prediction.currentTime, low: prediction.currentTime, high: prediction.currentTime },
    { day: 30, time: prediction.predictedTimes.days30, ...prediction.likelyRanges.days30 },
    { day: 90, time: prediction.predictedTimes.days90, ...prediction.likelyRanges.days90 },
    { day: 180, time: prediction.predictedTimes.days180, ...prediction.likelyRanges.days180 },
    { day: 365, time: prediction.predictedTimes.days365, ...prediction.likelyRanges.days365 }
  ].map((point) => ({
    day: point.day,
    time: point.time,
  ...
  return {
    horizonDays,
    predictedTime: round(interpolate(start.time, end.time, progress), 2),
